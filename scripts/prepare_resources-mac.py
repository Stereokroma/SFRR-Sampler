...
def main():
  ...
  # Read BINARY_NAME from the project-local xcconfig.
  # parse_xcconfig stops at #include, so we scan the file directly.
  BINARY_NAME = config['BUNDLE_NAME']
  local_xcconfig_path = os.path.join(projectpath, 'config', config['BUNDLE_NAME'] + '-mac.xcconfig')
  if os.path.exists(local_xcconfig_path):
    with open(local_xcconfig_path, 'r') as f:
      for line in f:
        line = line.strip()
        if line.startswith('BINARY_NAME'):
          parts = line.split('=', 1)
          if len(parts) == 2:
            BINARY_NAME = parts[1].strip()
            break
...
# VST3
  ...
  vst3['CFBundleExecutable'] = BINARY_NAME
...
# CLAP
  ...
  clap['CFBundleExecutable'] = BINARY_NAME
...
# AUDIOUNIT v2
  ...
  auv2['CFBundleExecutable'] = BINARY_NAME
...
# AUDIOUNIT v3
  ...
  auv3['CFBundleExecutable'] = BINARY_NAME
...
# AAX
  ...
  aax['CFBundleExecutable'] = BINARY_NAME
...
# APP
  ...
  macOSapp['CFBundleExecutable'] = BINARY_NAME
  ...
  macOSapp['CFBundleName'] = BINARY_NAME
